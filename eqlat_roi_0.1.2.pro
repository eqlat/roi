;--------------------------------------------------------------------------------------------------------------
;Copyright (C) 2012  Juan Antonio Anel, Guadalupe Saenz, Laura de la Torre and Doug Allen.
;
...
;     Last modified May 22, 2014 Juan Antonio Anel, Guadalupe Saenz Laura de la Torre and Doug Allen.

;Please report any bugs to Juan Anel (juan.anel@smithschool.ox.ac.uk) and Guadalupe Saenz (lusaga81@gmail.com). 
;******************************************************************************************************************

pro eqlat_roi_corr,PV_theta,lon,lat,PV_fixed,equivalat
...
;-------------------------------------------------------------------------------------------------------------------
   if (info(interesofarea).type eq 1) then begin
	x=dblarr(info(interesofarea).N+1) 
	y=dblarr(info(interesofarea).N+1)
...
;--------------------------------------------------------------------------------------------------------------------
;The contour is closed usando points with high resolution and the area enclosed is calculated with the function ROI. 
;--------------------------------------------------------------------------------------------------------------------
     if(x(0) eq x(n_elements(x)-1)) then begin
        points=abs(1.0*floor(10*(y(n_elements(y)-1)-(y(0)))))
        xclosed=fltarr(n_elements(x)+points) & yclosed=fltarr(n_elements(y)+points)
        xclosed(0:n_elements(x)-1)=x
        xclosed(n_elements(x):n_elements(x)+points-1)=x(0)
   
         if y(0) lt y(n_elements(y)-1) then begin
           yclosed(0:n_elements(y)-1)=y
           yclosed(n_elements(y):n_elements(y)+points-1)=reverse(indgen(points)*0.1+y(0))
         endif else begin
           yclosed(0:n_elements(y)-2)=y(0:n_elements(y)-2)
           yclosed(n_elements(y)-1:n_elements(y)+points-1)=indgen(points+1)*0.1+y(n_elements(y)-1)
         endelse
        
      add_sub,xclosed,yclosed,x,y,0,lon1,lat1,PV_theta1,PV_fixed,partialarea
;----- sum of total area----------------------------------------------------------------------------------------------
        totalarea=totalarea+partialarea
     endif else begin
;---------------------------------------------------------------------------------------------------------------------
...
;----------------------------------------------------------------------------------------------------------------------
;The contour is closed using a lot of points with high resolution of 0.1 degree the latitude until pole (North or South). 
;Finally the enclosing area is calculated using funtion ROI.
;---------------------------------------------------------------------------------------------------------------------
        if PV_fixed gt 0 then begin 			
	   beginningpoints=abs(1.0*floor(10.0*(90-(y(0)))))
	   finalpoints=abs(1.0*floor(10.0*(90-y(n_elements(y)-1))))
	endif
	if PV_fixed le 0 then begin
           beginningpoints=abs(1.0*floor(10.0*(-90-(y(0)))))
           finalpoints=abs(1.0*floor(10.0*(-90-y(n_elements(y)-1))))
        endif
	...
	xclosed(beginningpoints:beginningpoints+n_elements(x)-1)=x 
		...
		if (PV_fixed gt 0) then yclosed(beginningpoints+n_elements(y):n_elements(yclosed)-2)=reverse(90.-indgen(finalpoints+1)*0.1)
		if (PV_fixed le 0) then yclosed(beginningpoints+n_elements(y):n_elements(yclosed)-2)=y(n_elements(y)-1)-indgen(finalpoints+1)*0.1
        xclosed(n_elements(xclosed)-1)=xclosed(0) 
		if PV_fixed gt 0 then yclosed(n_elements(yclosed)-1)=90.
		if PV_fixed le 0 then yclosed(n_elements(yclosed)-1)=-90.
...
endelse
     ...
     for ila=0,n_elements(i_lat)-1 do begin
       ...
       if c_ext gt 0 and c_int gt 0 then begin
         ...
         Obj_Destroy, object
         if PV_fix gt 0 and PV_exterior gt PV_interior then partialarea1=-partialarea1
         if PV_fix le 0 and PV_exterior lt PV_interior then partialarea1=-partialarea1
       endif
       if c_ext eq 0 and c_int gt 0 then begin
         ...
         if partialarea1 lt 0 then partialarea1=-partialarea1
          Obj_Destroy, object
          if PV_fix gt 0 and PV_interior lt PV_fix then partialarea1=-partialarea1
          if PV_fix le 0 and PV_interior gt PV_fix then partialarea1=-partialarea1
       endif
       if c_ext gt 0 and c_int eq 0 then begin
          ...
          if partialarea1 lt 0 then partialarea1=-partialarea1
         Obj_Destroy, object
         if PV_fix gt 0 and PV_exterior gt PV_fix then partialarea1=-partialarea1
         if PV_fix le 0 and PV_exterior lt PV_fix then partialarea1=-partialarea1
       endif
