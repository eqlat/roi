;--------------------------------------------------------------------------------------------------------------
;Copyright (C) 2013  Juan Antonio Añel, Guadalupe Sáenz, Laura de la Torre and Doug Allen.
;
...
;     Last modified July 26, 2013 Juan Antonio Anel, Guadalupe Saenz Laura de la Torre and Doug Allen.

;Please report any bugs to Juan Anel (juan.anel@smithschool.ox.ac.uk) and Guadalupe Saenz (lusaga81@gmail.com). 
;******************************************************************************************************************

pro eqlat_roi,PV_theta,lon,lat,PV_fixed,equivalat,time_a
timer,/start
;----------constant---------------------------------------------------------------------------------------------------
...
;------------------------------------------------------------------------------------------------------
;The contour is closed using a lot of points with high resolution of 0.1 degrees
;of latitude to the pole (North or South). 
...
;------------------------------------------------------------------------------------------------------
	...
	xclosed(beginningpoints:beginningpoints+n_elements(x)-1)=x 
		...
		if (PV_fixed ge 0) then yclosed(beginningpoints+n_elements(y):n_elements(yclosed)-2)$
		=reverse(89.99-indgen(finalpoints+1)*0.1)
		if (PV_fixed lt 0) then yclosed(beginningpoints+n_elements(y):n_elements(yclosed)-2)$
		=y(n_elements(y)-1)-indgen(finalpoints+1)*0.1
